...
func populate_map_list():
	...
	var owned_maps = await MapStorage.list_owned_maps_async().completed
	
	for map_id in owned_maps:
		var inst = MapItem.instantiate()
		container_maps.add_child(inst)
		inst.populate(owned_maps[map_id],map_id)
		inst.connect("open_editor",Callable(self,"_on_MapItem_open_editor"))
		inst.connect("delete",Callable(self,"_on_MapItem_delete"))
		inst.connect("practice",Callable(self,"_on_MapItem_practice"))
		items.append(inst)
