...
func _ready():
	...
	if not params.has("gamemode"):
		text_result.text = "Your match did not specify a gamemode :( Cant show any info (This is an error)"
		return
...
func _on_BtnPublish_pressed():
	var result = await MapStorage.publish_map_async(map_id).completed
