...
Exits to:
	- MatchEnd
		...
		- turn_count: int
		- map_metadata: dict
	- EditorMenu (checked cancel)
	- Editor (checked cancel with verifiying == true)
		- load_map_id: String
...
func spawn_ball():
	...
	local_ball.connect("turn_completed",Callable(self,"_on_Ball_turn_completed"))
	local_ball.connect("reached_finish",Callable(self,"_on_Ball_reached_finish"))

	match_camera.follow(local_ball,false,false)

func load_map()->void:
	var map_jstring = await MapStorage.load_map_async(map_id, creator_id).completed
