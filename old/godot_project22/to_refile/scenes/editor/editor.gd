...
func _ready():
	...
	if params.has("load_map_id"):
		var map_jstring = await MapStorage.load_map_async(params["load_map_id"]).completed
...
func _process(delta):
	...
	update()
	var coord = tilemap_cursor.local_to_map(get_global_mouse_position())
...
#		if event.is_action_just_pressed("editor_tool_use"):
		...
		if event.pressed:
			tool_dragged = true
			var cell = tilemap_cursor.local_to_map(get_global_mouse_position())
			tool_dragged_from = tilemap_cursor.map_to_local(cell)
			tool_use(reverse)
...
# :param reverse: reverse the tool (remove tile)
func tool_use(reverse) -> void:
...
func save_map_async():
	...
	if current_metadata.has("id"):
		...
		map_id = String(Time.get_unix_time_from_system())  # TODO replace with proper uid gen (maybe by server request)
	map.update_metadata(map_id, map_name, creator_id, creator_display_name)

	# export map
	var map_jstring = map.serialize()
	var ack = await MapStorage.save_map_async(map_id, map_jstring).completed
	if ack.is_exception():
...
func _tilemap_set_line(tilemap, start_world: Vector2, end_world: Vector2, tile: int):
	var from_cell = tilemap.local_to_map(start_world)
	var to_cell = tilemap.local_to_map(end_world)
	...
	for i in range(line_vec.length() + 1):
		if tilemap.has_method("editor_tile_change"):
			...
			var world = map.map_to_local(from_cell + i * (line_vec / line_vec.length()))
...
func _tilemap_set_flood(map, from_world: Vector2, tile: int):
	var from_cell = map.local_to_map(from_world)
	var x = from_cell.x
	var y = from_cell.y
	var new_tile = tile
	...
	while not stack.is_empty():
		...
		while x1 < w and map.get_tile_id_at_cell(Vector2(x1, y)) == old_tile:
			var world = map.map_to_local(Vector2(x1, y))
...
func _on_BtnSave_pressed():
	...
	var ack = await save_map_async().completed
	if not ack.is_exception():
		get_tree().change_scene_to_file("res://scenes/editor_menu/EditorMenu.tscn")
...
func _on_BtnPublish_pressed():
	...
	await save_map_async().completed
	var params = {
	...
	get_tree().change_scene_to_file("res://scenes/match_practice/MatchPractice.tscn")
