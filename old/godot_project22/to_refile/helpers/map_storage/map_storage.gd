...
func load_map_async(map_id:String, owner_id:String=""): # -> map_jstring
	...
	var map_jstring = await _load_from_server_async(map_id,owner_id).completed
	return map_jstring
...
func save_map_async(map_id:String, map_jstring:String): # -> ApiStorageObjectAck/s
	...
	var ack = await _save_to_server_async(map_id, map_jstring, false).completed
	return ack
...
func publish_map_async(map_id:String):
	...
	return await Networker.rpc_call("publish_map", JSON.stringify({"map_id": map_id})).completed
...
func list_owned_maps_async()->Dictionary:
	var result = await Networker.collection_list_owned_objects_async(Global.MAP_COLLECTION).completed
	
	var owned_maps = {}
	for i in result:
		var test_json_conv = JSON.new()
		test_json_conv.parse(i.value).result["metadata"]["name"]
		owned_maps[i.key] = test_json_conv.get_data()
...
func list_public_maps_async()->Array: # ->
	var result = await Networker.collection_list_public_objects_async(Global.MAP_COLLECTION).completed
	
	var public_maps:Array
	for i in result:
		var test_json_conv = JSON.new()
		test_json_conv.parse(i.value).result
		var mapdict = test_json_conv.get_data()
...
func _load_from_server_async(map_id:String, owner_id:String="")->String: # -> map_jstring
	var object
	if owner_id == "":
		object = await Networker.collection_read_object_async(Global.MAP_COLLECTION,map_id).completed
	else:
		object = await Networker.collection_read_object_async(Global.MAP_COLLECTION,map_id, owner_id).completed
...
func _save_to_server_async(map_id:String, map_jstring:String, public:bool=false): # -> ApiStorageObjectAck/s
	return await Networker.collection_write_object_async(Global.MAP_COLLECTION, map_id, map_jstring, public).completed
