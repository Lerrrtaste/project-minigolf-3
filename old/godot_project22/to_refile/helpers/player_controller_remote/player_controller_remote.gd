...
##
## Acts checked behalf of a another local player controller
...
func _ready():
	Networker.connect("match_state",Callable(self,"_on_Networker_match_state")) #warning-ignore:return_value_discarded
...
func _on_Networker_match_state(state:NakamaRTAPI.MatchData)->void:
	...
	if state.op_code == Global.OpCodes.BALL_IMPACT:
		...
		var impact:Vector2 = str_to_var(data_dict["impact_vec"])
		active = false
	...
	if state.op_code == Global.OpCodes.BALL_SYNC:
		...
		test_json_conv.parse(state.data).result
		var data_dict = test_json_conv.get_data()
		var pos:Vector2 = str_to_var(data_dict["synced_pos"])
