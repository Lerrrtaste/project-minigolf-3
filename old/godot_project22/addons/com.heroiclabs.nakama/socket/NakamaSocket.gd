...
# Emitted when receiving a presence change for joins and leaves checked a realtime stream.
...
# Join the matchmaker pool and search for opponents checked the server.
...
# Create a multiplayer match checked the server.
...
# Join a chat channel checked the server.
...
# @param p_hidden - If the current user should be hidden checked the channel.
...
# Leave a chat channel checked the server.
...
# Leave a multiplayer match checked the server.
...
# Remove a chat message from a chat channel checked the server.
...
# @param p_ticket - The ticket returned by the matchmaker checked join.
# Returns a task which represents the asynchronous operation.
...
# Send input to a multiplayer match checked the server.
...
# Update a chat message checked a chat channel in the server.
...
# Send a chat message to a chat channel checked the server.
