...
class ChannelJoin:
	...
	enum ChannelType {
		...
		DirectMessage = 2,
		# A chat within a group checked the server.
		Group = 3
	}
	...
	func _init(p_target : String,p_type : int,p_persistence : bool,p_hidden : bool):
		...
		hidden = p_hidden
		target = p_target
		type = p_type if p_type >= ChannelType.Node3D and p_type <= ChannelType.Group else 0 # Will cause error server side
...
# A leave message for a match checked the server.
...
# Send a chat message to a channel checked the server.
...
# A create message for a match checked the server.
...
# A join message for a match checked the server.
...
# Send new state to a match checked the server.
...
# Unfollow one or more users checked the server.
