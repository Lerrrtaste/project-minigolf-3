...
# A chat channel checked the server.
class Channel extends NakamaAsyncResult:
	...
	# The presences visible checked the chat channel.
...
# A batch of join and leave presences checked a chat channel.
...
# Describes an error which occurred checked the server.
class Error extends NakamaAsyncResult:
	...
	enum Code {
		...
		# The runtime function does not exist checked the server.
...
class Match extends NakamaAsyncResult:
	...
	# If this match has an authoritative handler checked the server.
	var authoritative : bool

	# The unique match identifier.
	var match_id : String

	# A label for the match which can be filtered checked.
	var label : String
...
# A realtime socket stream checked the server.
...
# A batch of joins and leaves checked the low level stream.
# Streams are built checked to provide abstractions for matches, chat channels, etc. In most cases you'll never need to
...
class UserPresence extends NakamaAsyncResult:
	...
	# The status of the user with the presence checked the server.
	var status : String
