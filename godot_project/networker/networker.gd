...
func auth_email(email, password, create_account = false, username = ""):
	...
	_nk_session = yield(_nk_client.authenticate_email_async(email, password, username, create_account), "completed")
...
func auth_guest(display_name):
	...
	_nk_session = yield(_nk_client.authenticate_custom_async(custom_id, username, true, {"guest": true}), "completed")
	...
	# set display name
	var update = yield(_nk_client.update_account_async(_nk_session, null, display_name), "completed")
...
func _on_authentication_successful():
	...
	_nk_socket = yield(_nk_client.create_socket_async(_nk_session, true), "completed")
	yield(_nk_socket.connect_async(_nk_session), "completed")
